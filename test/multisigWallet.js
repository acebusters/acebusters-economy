...
contract('MultisigWallet', function(accounts) {
  ...
  it('should send balance to passed address upon death', async function() {
    ...
    let wallet = await MultisigWalletMock.new(accounts, ownersRequired, {value: web3.toWei('10', 'ether')});
  ...
  it('should execute transaction if above daily limit and enough owners approve', async function() {
    ...
    let txnHash = await wallet.execute(accounts[2], 11, hash, {from: accounts[0]});
    ...
    txnHash = await wallet.execute(accounts[2], 11, hash, {from: accounts[1]});
