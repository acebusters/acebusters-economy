...
contract MockPowerEventReplacement is PowerEventReplacement {
    ...
    function MockPowerEventReplacement(address _controllerAddr, uint256 _startTime, uint256 _minDuration, 
    uint256 _maxDuration, uint256 _softCap, 
    uint256 _hardCap, uint256 _discount, uint256 _amountPower, 
    address[] _milestoneRecipients, uint256[] _milestoneShares)
    ...
    function startCollection() isState(EventState.Waiting) {
        ...
        // move ceiling
        uint256 newCeiling = ceiling.mul(discountRate).div(rateFactor);
