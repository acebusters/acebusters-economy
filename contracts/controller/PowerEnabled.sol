...
contract PowerEnabled is MarketEnabled {
    ...
    function PowerEnabled(address _powerAddr, address _pullAddr, address _storageAddr, address _nutzAddr) public
    ...
    function slashDownRequest(address _holder, uint256 _value, bytes32 _data) public onlyAdmins {
    ...
    function _vestedDown(uint256 _total, uint256 _left, uint256 _start, uint256 _now) internal returns (uint256) {
        if (_now <= _start) {
            return 0;
        }
