...
contract PowerEvent {
    ...
    uint256 public rateFactor = 1000000;
    ...
    function stopCollection() public isState(EventState.Collecting) {
    ...
    function completeFailed() public isState(EventState.Failed) {
        var contr = Controller(controllerAddr);
    ...
    function completeClosed() public isState(EventState.Closed) {
        ...
        // move ceiling
        uint256 ceiling = contr.ceiling();
        uint256 newCeiling = ceiling.mul(rateFactor).div(discountRate);
        contr.moveCeiling(newCeiling);
        // dilute power
